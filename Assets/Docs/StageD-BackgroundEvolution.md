...
| 7 | **归档时观察到的一条环境现象（与本阶段功能无关，原因未定）** | `Clock_RunsAtTheRequestedRateAcrossRealFrames` 在**二进制未变**的情况下，全量套件进程内出现过 270 ms 与 455 ms 的单帧离群值（该断言的原始门槛是最差帧 ≤ 250 ms）；同一测试单独运行四次则是 241 帧、中位 8.33 ms、p90 8.4 ms、最差 11–30 ms、超过 50 ms 的帧 0 个。**离群值的成因没有查清**，因此：①**保留原始 250 ms 门槛**——一次离群就是一次失败，不拿没有依据的解释去放宽它；②新增**分布日志**（中位 / p90 / 最差帧及其帧序号 / 超过 50 ms 的帧数），让这种失败可诊断；③顺带把新增 4096² 测试的读数缓冲**复用**（把两次 64 MB 回读从新分配改为一共两块缓冲），并在 teardown 归还内存——这只是测试自身的开销卫生，**不是**对离群值的归因 |
...
| 项 | 内容 |
|---|---|
| **版本位置** | Git 标签 **`stage-d-background-evolution`** → 提交 **`94d92cb`**（评审签收的那一个提交，**功能验收以它为准**；现有标签与历史均未改动） |
| **标签版本 vs 后续测试维护** | 标签指向的 `94d92cb` 就是被签收的**功能版本**；本文件 §8 以及其后的若干次提交属于**归档与测试维护**（归档文本、测试描述收窄、分布日志、读数缓冲复用、teardown 归还内存）。这些提交**不改变运行时行为**，不构成新的功能验收对象，标签也**不随之移动**。§4 的性能数字与 §3 的测试结论都来自 `94d92cb` 的运行时行为 |
| **归档文本本身** | 本节写在该标签**之后**的提交里：标签指向的树里没有 §8，其余全部包含 |
...
| **归档时附带的两处测试维护** | ①`CpuFailure_ThenSwitchingToGpu_…` 的描述收窄为「错误清除、允许重新提交」（注入规则每次都抛异常，完整重试由第 16 条覆盖）；②`Clock_RunsAtTheRequestedRateAcrossRealFrames` **保留原来的最差帧 ≤ 250 ms 门槛**，另加分布日志（中位/p90/最差+帧序号/超限帧数），并把新增 4096² 测试的读数缓冲改为复用、teardown 归还内存。两者都只动测试，且**离群帧的成因未查清**，没有据此放宽任何门槛 |
