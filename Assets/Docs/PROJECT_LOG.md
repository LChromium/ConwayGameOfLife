...
### T7 — 窄窗口的响应式布局未实现（R3 新增）

详见 R3 章节。摘要：内容实测需 949px 而参考高度为 900px；已做多处收紧并给样本列表加滚动，640×480 下通过断言，但矮窗口会优先挤压装饰性 footer。真正的窄屏纵向堆叠布局尚未实现。

### T8 — 渲染成本仍未采样（R3 新增）

`TechnicalAnalysis.md` §7 已从"渲染是真瓶颈"降级为"结构性风险（尚未量化）"。需要在 Unity Profiler 中分别采样规则推进、网格重绘、UI 布局三段，才能决定优化方向。**在此之前不应实施 §6 的任何优化。**
