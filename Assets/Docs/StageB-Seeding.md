...
**「GPU 独立生成噪声」是远期可选实验，不是阶段 C 的内容。** 它只有在
（a）大棋盘基准显示噪声生成真的成了瓶颈，且（b）浮点一致性的控制与验证方案先立起来，
这两件事都成立之后才值得做。阶段 C 优先做**大棋盘基准**；CPU 单生成器继续保留。
...
### 4.2 两个问题：内容对不对，任务还算不算数

采纳或拒绝一个结果要同时回答两个**互相独立**的问题：

| 问题 | 判据 | 覆盖什么 |
|---|---|---|
| 产出的盘面还描述当前控件吗 | 参数快照 `finishedParameters.Equals(parameters)` | 用户改了参数；结果与控件不一致 |
| 这个任务还是当前那个请求吗 | 请求版本 `finishedVersion == requestVersion` | 取消、销毁、被新请求替代 |

**只有内容是不够的**，因为取消、销毁和「用同一组参数重新请求」都**不改变参数**：
内容检查会让一个已经被放弃的任务继续决定当前状态。版本在每次 `RequestCandidate`、
`Cancel`、`Apply`、`Dispose`、以及 `SetParameters` 真正改变参数时递增，
每一个任务在启动时捕获自己的版本，任务结束时（无论成功还是失败）拿它比对。

`SetParameters` 因此**当场**让在途请求失效（版本递增，同时清掉挂起请求与已完成结果）：
已经跑起来的任务可以跑完，但它产出的是控件已经离开的那组参数、属于一个已经作废的请求，
到达时被丢弃，而不是下一帧被上传显示。
...
**失败与成功走同一条路径、同一套判据。** 已被取代或被取消的任务抛异常时，
它的失败和它的结果一样被丢弃：既不写 `FailureMessage`（当前请求并没有失败），
**也不清掉取代它的那个请求**。异常分支曾经无条件复位 `wantCandidate`/`requestPending`，
「旧任务崩溃」因此把用户刚发出的新请求一起取消；只按参数快照判断也不够——
取消后紧接着用同样参数重新请求时，快照完全相同，被放弃的任务会把新请求一起带走。

跨会话生命周期：`OnDestroy` 调用 `Seeding.Dispose()`，**不在主线程等待任务结束**——
退出路径阻塞等一个没人要的结果会把干净退出变成卡顿。任务自己跑完，成功与失败都被
身份检查拒绝。
...
| **取消后旧任务抛异常** | 参数没变，所以只有请求版本能拒绝它。不报失败、不产生候选、不影响任何后续请求 |
| **取消后立刻用同一组参数重新请求** | 新请求有自己的版本；旧任务无论成功还是失败都属于已经结束的那个请求。新请求**自己算一遍**，不复用旧结果 |
...
### 测试（EditMode 78/78，PlayMode 39/39）
...
| **取消后旧任务失败不报错** | `Cancel_ThenTheAbandonedTaskFails_ReportsNothing`（参数没变，所以这条只有请求版本能通过：连泵 10 次，`FailureMessage` 必须始终为空） |
| **取消后立刻用同一组参数重新请求：旧任务失败不得取消新请求** | `Cancel_ThenAnIdenticalRequest_IsNotCancelledByTheAbandonedFailure`（新请求在**旧任务结束之前**发出并断言已挂起；旧任务随后抛异常；新请求必须自己算一遍并落地） |
| —— 同步检查旧**成功**结果 | `Cancel_ThenAnIdenticalRequest_DoesNotAdoptTheAbandonedSuccess`（旧任务成功产出的正是新请求要的那块盘面，仍然不得采用；用运行次数证明新请求自己算了一遍） |
| 当前请求失败仍然上报并释放 | `TheCurrentTaskFailing_StillReportsAndReleases`（防止上一条被修成「什么都不清」，那会把面板焊死） |
| —— 这三条确实是在测身份，不是在测内容 | **证伪**：把两处判据都改回「只比参数快照」，`unity test . --mode EditMode --filter ConwayGameOfLife.Tests.LifeSeedingSessionTests` 得到 `22 passed / 3 failed`——失败的正好是上面三条，症状分别是「旧成功被采纳」「新请求等不到自己的计算（超时）」「被撤回的请求报了失败」；同目录其余 22 条在内容判据下**照样通过**，说明它们不覆盖身份。改回双判据即 78/78 |
...
| 噪声在 CPU 生成 | GPU 不独立生成噪声，这是本阶段的选择。见 §3：GPU 独立生成属**远期可选实验**，不在阶段 C 范围 |
| 未做亚像素密度总览 | 属阶段 C |
...
| 项 | 内容 |
|---|---|
| **版本位置** | Git 标签 `stage-b-life-seeding`（主体验收）与 `stage-b-life-seeding-r2`（请求身份补丁，补在它之后；旧标签保留，历史不重写）。提交号用 `git describe --tags` 取，不写死在文档里 |
| **范围** | fBM + 域扭曲概率播种：生成器、预览/应用/取消/重置语义、后台生成与界面约束、工具区页签 |
| **测试报告** | EditMode **78/78**、PlayMode **39/39**（`test-results-*.xml` 随版本保存）|
| **证伪记录** | §5 两条：请求身份（内容判据下 22 passed / 3 failed，失败的正好是身份三条）、过期失败清除新请求。`tr-*.xml` 按 `.gitignore` 是本地临时输出，因此记录的是**命令与观测**，不是文件 |
...
**阶段 C 的内容**：**大棋盘基准**优先（棋盘规模、后端、帧成本、生成耗时的分项口径），
以及亚像素密度总览。**CPU 单生成器继续保留**，本阶段不因基准而改动播种路径。

**GPU 独立生成噪声是远期可选实验**，不是阶段 C 的内容：它要同时满足
（a）基准显示噪声生成确为瓶颈、（b）浮点一致性的控制与验证方案先立起来，
这两条都不在今天的工作范围内。
