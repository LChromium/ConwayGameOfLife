...
**当前结果：EditMode 86/86 通过，PlayMode 62/62 通过。**
...
- **后台演化**（PlayMode，`LifeBackgroundEvolutionTests` 19 条）：CPU 规则在 worker 上算，主线程只接管完整世代。真线程池下验证「计算期间主线程读到的仍是已接管的那一代」与「算完但未接管的棋盘不可见」；用**可控任务**（调度器与规则步进都可注入）验证暂停冻结＋暂存、恢复按顺序接管、单步优先接管、重置/切换后端/销毁不让旧结果落地；**边界切换**（在途 / 待接管两种时机）与**任务失败**（先推进再抛异常）都要求「拒绝之后从显示棋盘重建」，并与同步参考逐格一致。两条原则由后端自己保证：**失败状态下 `Step()` 在同锁内拒绝提交**（只有显式清除或换盘能解除），以及**界面错误属于当前使用的后端**（切后端同帧同步读数与 tooltip，不留一帧窗口）。五条证伪各只改一处，分别让 5 / 2 / 2 / 1 / 1 条失败。
...
│   └── PlayMode/                  # 界面自举、交互、布局与文字适配、性能口径、时钟过载、后台演化（62 项）
...
> 任务失败按会话身份处理，**失败状态下后端自己拒绝提交**（同锁内判定，不靠调用方再补一次检查），
> 当前失败停止自动提交、显示停在最后一个完整世代，只有显式重试或换盘才解除；
> **界面错误属于当前后端**——切到 GPU 时同帧丢掉 CPU 的旧错误，切回时按 CPU 的当前状态显示。
